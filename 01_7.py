...
# If requires_grad is set to True, PyTorch will track all operations on this tensor, allowing for automatic differentiation.
# If false, PyTorch will not track operations on this tensor, and it will not be part of the computation graph for gradient calculation.

# 가중치 텐서 (기울기 추적)
w = torch.tensor([0.5, 0.2, -0.3], requires_grad=True)   # shape: (3,) 미분 추적 활성화.

# 편향
b = torch.tensor(0.1, requires_grad=True)                # shape: ()

# 선형 결합
y = x @ w + b                                            # inner product(x1w1 + x2w2 + x3w3) + bias, shape: ()
print("PyTorch output:", y.item())

# 자동 미분
y.backward()                                             # 역전파 알고리즘으로 기울기 계산. y.backward를 한 시점에서 x, w, b에 대한 미분이 계산됨

# Check Gradients, gradients are stored in the .grad attribute of the tensors.
# Gradients are None if requires_grad is False or if backward() has not been called yet.
print("grad w:", w.grad)                                 # dy/dw -> partial derivative of y with respect to w
print("grad b:", b.grad)                                 # dy/db -> partial derivative of y with respect to b
